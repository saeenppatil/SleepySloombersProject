...
class Method_GCN(method, nn.Module):
    def __init__(self, mName, mDescription):
        ...
        self.hidden_dim = 128 # less dimensions
        self.dropout_rate = 0.4 # higher dropout
        ...
        # Model architecture chnaged (3 GCN layers + BatchNorm)
        self.gc1 = GCNLayer(500, self.hidden_dim)
        self.bn1 = nn.BatchNorm1d(self.hidden_dim)
        self.gc2 = GCNLayer(self.hidden_dim, self.hidden_dim)
        self.bn2 = nn.BatchNorm1d(self.hidden_dim)
        self.gc3 = GCNLayer(self.hidden_dim, 3)

    def forward(self, x, adj):
        x = self.gc1(x, adj)
        x = self.bn1(x)
        x = F.relu(x) # also changed from elu to relu 
