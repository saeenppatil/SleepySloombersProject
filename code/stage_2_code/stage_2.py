...
class Method_MLP(method, nn.Module):
    ...
    max_epoch = 300
    learning_rate = 1e-3
    weight_decay = 1e-4  # <-- new weight decay term
    ...
    def __init__(self, mName, mDescription):
        ...
        # Changed model: deeper, wider, added Dropout layers
    ...
    def train(self, X, y):
        # Optimizer changed from Adam -> AdamW with weight decay 
        ...
        test1, test2 = self.test(self.data['test']['X'], raw=True)
        final_test_loss = self.loss_function(torch.FloatTensor(test2), torch.LongTensor(self.data['test']['y'])).item()
        final_test_accuracy = accuracy_score(test1, self.data['test']['y'])
